#!/bin/sh
# Builds IntPortal.app and packages it into a distributable .dmg with an
...
# output is named IntPortal-<version>-with-AI.dmg instead — everything
...
STAGE="$(mktemp -d)/IntPortal"
SUFFIX=""
[ -n "${BUNDLE_LLAMA_SERVER:-}" ] || [ -n "${BUNDLE_MODELS:-}" ] && SUFFIX="-with-AI"
DMG="$ROOT/dist/IntPortal-$VERSION$SUFFIX.dmg"
...
mkdir -p "$ROOT/dist"
rm -f "$DMG"
hdiutil create -volname "IntPortal $VERSION$SUFFIX" \
